...
float ledPattern(vec2 uv) {
  const int numBlocks = 48;
  vec2 translatedUV = (uv - vec2(0.1, 0.1)) / 0.3;
  vec2 scaledUV = translatedUV * float(numBlocks);
  vec2 tileUV = vec2(fract(scaledUV.x), fract(scaledUV.y));

  return mapRange(distance(tileUV, vec2(0.5, 0.5)), 0.125, 0.33, 0.0, 1.0);
}
// </TacTicOe>
...
void main() {
  ...
  if (vMyUv.x < 0.5) {
    float led = ledPattern(vMyUv);

    intensity = mix(mapRange(led, 0.0, 1.0, 0.25, 1.0), mapRange(led, 0.0, 1.0, uPlayer == 1 ? 3.0 : 1.5, 0.25), uEmissiveIntensity / uMaxIntensity);
  } else {
    intensity = 0.25;
  }
